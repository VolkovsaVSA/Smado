    ...
    var body: some View {
        ...
        Button {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let images = doc.images?.allObjects as? [ImageCD] {
                    ...
                    if images.isEmpty {
                        ...
                        ZStack {
                            ForEach(images.count > 3 ? Array(0...2).indices : images.indices, id:\.self) { index in
                                if index < images.count {
                                    DocThumbnail(image: images[index], cellWidth: frameWidth)
